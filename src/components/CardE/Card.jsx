...
const Card = ({ data }) => {
  const { id, image, title, color, totalPrice, status, date } = data;
  return (
    <Link href={`/orders/details/${id}`}>
      <Stack direction="row" className={styles.card}>
        <Stack
          direction={"row"}
          sx={{ width: "60%", justifyContent: "flex-start" }}
          gap={1.5}
        >
        ...
        <Typography className={styles.price} style={{ margin: "15px 0px" }}>
          ₹ {totalPrice}
        </Typography>
        <Stack
          style={{ margin: "15px 20px 15px 0px", gap: "1px", width: "170px" }}
        >

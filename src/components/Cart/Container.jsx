...
const Container = () => {
    ...
    // useEffect(() => {
    //     console.log("Cart id: ", selectedCartItemId);
    // }, [selectedCartItemId]);
    ...
    return (
        <Stack direction={"column"} className={styles.container} gap={3} style={{ marginBottom: "30px" }}>
            <Typography className={styles.title}>MY SHOPPING CART {Array.isArray(cart.items) && `[ ${cart.items.length} ]`}</Typography>
            <Stack direction={"column"} gap={1.8}>
                {
                    Array.isArray(cart.items) && cart.items.length > 0 ? cart.items.map((item, index) => (
                        <Card 
                            data={item} 
                            key={index} 
                            edit={false} 
